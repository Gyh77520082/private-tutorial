...
if [ $newpass = $renewpass -a $newrootpass = $renewrootpass ];then
	for ip in  192.168.0.{154,198,21,43,53,57}
	do

	{
		expect ./update_passwd.exp $ip $pass $newpass $newrootpass
		echo "$ip lmode新密码：$newpass" root新密码：$newrootpass >> /home/lmode/updatepasswd/newpassword.txt
	}
